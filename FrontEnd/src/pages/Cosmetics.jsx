...
function Cosmetics() {
  ...
  const handleWhatsAppBuy = async (item) => {
    ...
    if (item.qty < qty) {
      ...
      if (!newWindow) {
        alert("Popup blocked! Please allow popups or click the link: " + whatsappLink);
      }

      try {
        await fetch('http://localhost:5000/cosmetics/update-stock', {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="bg-white text-green-800 min-h-screen py-24">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl font-extrabold text-center mb-16 text-green-700">Our Cosmetic Products</h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {cosmetics.map((item) => (
            <div key={item.id} className="bg-white border p-6 rounded-xl shadow hover:shadow-green-300 transition-all">
              <img src={`http://localhost:5000${item.image}`} alt={item.name} className="rounded-xl mb-4 w-full h-56 object-cover" />
